...
def main():
    ...
    if result.get("success"):
        print(f"✅ Setup complete: status set to '{result['previous_status']}' -> '{result['target_status']}'")
    else:
    ...
    if result.get("success"):
        print(f"❌ TEST 1 FAILED: Update should have been blocked but succeeded!")
        ...
        print(f"✅ TEST 1 PASSED: Update blocked as expected")
    ...
    if not result.get("success"):
        print(f"❌ TEST 2 FAILED: Update should have succeeded but was blocked!")
        print(f"   Result: {result}")
        return 1
    else:
        print(f"✅ TEST 2 PASSED: Update succeeded as expected")
        print(f"   Action: {result.get('action')}")
