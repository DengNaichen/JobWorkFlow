...
class TestProcessItemTailoring:
    ...
    def test_tracker_not_found_error(self, mock_parse_tracker):
        ...
        mock_parse_tracker.side_effect = ToolError(
            ...
            message="Tracker file not found: trackers/missing.md"
        )
...
class TestCareerTailorTool:
    ...
    @patch("tools.career_tailor.compile_resume_pdf")
    @patch("tools.career_tailor.verify_pdf_exists")
    def test_boundary_no_db_or_tracker_status_mutation(self, mock_verify_pdf, mock_compile_pdf, tmp_path):
...
"""
        ...
        resume_template_path.write_text("\\documentclass{article}\\begin{document}OK\\end{document}", encoding="utf-8")
