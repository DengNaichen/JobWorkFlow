...
class TestBootstrapSchema:
    ...
    def test_creates_jobs_table(self, tmp_path):
        ...
        required_columns = {
            ...
            "payload_json", "created_at", "updated_at", "resume_pdf_path",
            "resume_written_at", "run_id", "attempt_count", "last_error"
        }
...
class TestJobsIngestWriter:
    ...
    def test_insert_with_custom_status(self, tmp_path):
        ...
        # Verify status
        conn = sqlite3.connect(str(db_path))
        cursor = conn.execute("SELECT status FROM jobs WHERE url = ?", ("https://example.com/job1",))
...
class TestBoundaryBehavior:
    ...
    def test_insert_only_no_updates(self, tmp_path):
        ...
        with JobsIngestWriter(str(db_path)) as writer:
            inserted, duplicates = writer.insert_cleaned_records([duplicate_record], status="shortlist")
            writer.commit()
    ...
    def test_no_delete_operations(self, tmp_path):
        ...
        for i in range(5):
            cursor = conn.execute("SELECT COUNT(*) FROM jobs WHERE url = ?", (f"https://example.com/job{i}",))
            assert cursor.fetchone()[0] == 1
