    ...
    try:
        ...
        if not isinstance(payload, dict):
            raise create_validation_error(
                "Invalid cursor format: payload must be a JSON object"
            )

        if "captured_at" not in payload:
            raise create_validation_error(
                "Invalid cursor format: missing 'captured_at' field"
            )

        if "id" not in payload:
            raise create_validation_error(
                "Invalid cursor format: missing 'id' field"
            )
        ...
        if not isinstance(captured_at, str):
            raise create_validation_error(
                "Invalid cursor format: 'captured_at' must be a string"
            )

        if not isinstance(record_id, int):
            raise create_validation_error(
                "Invalid cursor format: 'id' must be an integer"
            )
        ...
        raise create_validation_error(
            f"Invalid cursor format: malformed JSON - {str(e)}"
        ) from e
        ...
        raise create_validation_error(
            f"Invalid cursor format: malformed base64 - {str(e)}"
        ) from e
